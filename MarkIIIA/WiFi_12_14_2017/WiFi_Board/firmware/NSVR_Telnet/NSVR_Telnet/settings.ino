...
// N.B. This needs to be set to "sta" on the "Host" Adapter
...
// If this is not set, wifi immediately starts as an access point
// unless mode is 'sta'
// N.B. This needs to be set to the Suit MAC:ID on the "Host" Adapter
...
// N.B. This needs to be set to the base64(Suit MAC:ID) on the "Host" Adapter
...
// Default access point name includes MAC to support
// many things in the same area.
String setting_ap_ssid() {
    String ap_ssid;
    if (!Embedis::get("ap_ssid", ap_ssid)) {
        ap_ssid = String(F("NVSR:")) + WiFi.softAPmacAddress();
    }
    return ap_ssid;
}
...
String setting_mdns_hostname() {
    ...
    if (!Embedis::get("hostname", mdns_hostname)) {
        mdns_hostname = String(F("NVSR:")) + WiFi.softAPmacAddress();
    }
